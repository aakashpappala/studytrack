...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    long countByStudentIdAndAssignedDateAndStatus(Long studentId, LocalDate assignedDate, TaskStatus status);

    @Query("SELECT COUNT(t) FROM Task t WHERE t.status = 'COMPLETED' AND CAST(t.completedAt AS date) = :date")
    long countTasksCompletedOnDate(@Param("date") LocalDate date);

    @Query("SELECT t FROM Task t WHERE t.student.user.email = :email ORDER BY t.assignedDate DESC")
    List<Task> findByStudentEmail(@Param("email") String email);

    @Query("SELECT t FROM Task t WHERE t.student.user.email = :email AND t.assignedDate = :date")
    List<Task> findByStudentEmailAndAssignedDate(@Param("email") String email, @Param("date") LocalDate date);
}
